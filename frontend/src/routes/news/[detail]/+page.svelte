<script>
    ...
    let isLoadingForComment = false;
    let isSaveComment = false;
    let news;
    let comments;
    let author;
    ...
    const schema = yup.object({
        'comment': yup.string().required().min(7).max(200)
    });

    // ON COMMENT SUBMIT
    const onCommentSubmit = async () => {
        submitted = true;
        isLoadingForComment = true;
        ...
        if(isValid){
            await axios.post(`${API_URL}/comments`, {
                news_id: $page.params.detail,
                user_id: userdata.user_id,
                comment: fields.comment
            }, config).then(response => {
                if(response.data.code === 200){
                    isSaveComment = response.data;
                }
        ...
        isLoadingForComment = false;

        setTimeout(() => {
            getDatas($page.params.detail)
            isSaveComment = false;
            fields.comment = "";
            submitted = false;
        }, 750);
    ...
    <div class="content">
        ...
        <div class="left">
            ...
            <div class="bg-green-100 border border-green-400 mb-7 text-green-700 px-4 py-3 rounded relative" role="alert">
                <span class="block sm:inline">{isSaveComment.message}</span>
            </div>
            {/if}

            <Form {fields} {schema} submitHandler={onCommentSubmit} {submitted}>
                <div class="mb-6">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Comment Area</label>
                    <textarea bind:value={fields.comment} autocomplete="off" name="comment" cols="30" rows="10" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
